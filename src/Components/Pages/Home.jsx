  ...
  return (
    <div>
      <div className="w-11/12 mx-auto">
        ...
        {/*  */}
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 h-fit">
          {Server.map((item) => (
            <Link to={`/product/${item.id}`} key={item.id}>
              <div className="border p-4 rounded-lg shadow-md hover:shadow-xl transition">
                <img src={item.image} alt={item.name} className="w-full h-44 lg:h-96 object-cover mb-4 rounded" />
